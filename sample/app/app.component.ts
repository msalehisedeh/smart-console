...
export class AppComponent {
  ...
  constructor(
    ...
    this.smartService.redirectedOutput().subscribe((event) => {
  ...
  execute() {
    ...
    if (this.options.watch) {
      ...
      this.subscribers = [];
      list.map((key: any) => {
        const sbc = this.smartService.addWatch(key).subscribe((event: any) => {
          this.data.push(
            "Watch for '" +
              key +
              "' is reporting log:" +
    ...
    console.log(
      'level_3',
      'message that may not be as imortant',
      'additional info'
    );
    ...
    console.trace(
      'level_5',
      'message that may or may not be as imortant',
      'additional info'
    );
    console.warn(
      'level_5',
